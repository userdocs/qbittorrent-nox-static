...
🟥 Emulation requirements for cross building
	...
	- `qbt_with_qemu` - this defaults to `yes` but if set to `no` if will enable `_host_deps` version of the modules that need a host version to be built before they can cross build.
	...
	- `qbt_host_deps` - defaults to `no` but if set to yes it will not build locally but instead pull in prebuilt host deps for the these modules avoiding the need to build them locally.
		...
		- This makes build time equivalent to using `qemu` without the requirement of having `qemu`

	If you build using Qt6 you will need to have these dependencies installed on the host (not inside the docker), <Modal id="qemu" label="qemu and binfmt"/>
