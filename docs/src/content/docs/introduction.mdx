...
The script creates a fully static `qbittorrent-nox` binary using <Tooltip id="glibc" label="glibc"/>
...
You always have a easy method available to you to build your own releases, simply by forking the repo and using the available workflows. How to do this will be shown in the <Tooltip id="github-actions" label="Github Actions"/> sections later on. You can build locally or fork the repo and build and release using CI where the git repo acts as a local environment to the script.
:::
