...
`qbt-nox-static.bash™` was originally a simple, amateurish bash script to build a static `qbittorrent-nox` binary for `x86_64`. The script has grown and evolved since then and now it's a complicated, amateurish bash script.
