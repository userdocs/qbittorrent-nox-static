...
- Build it yourself locally
	...
	- Second, by not stripping the binary

		This sets `-Og` which does not optimise any debug sections but apps are not set to debug.

		```bash
		export qbt_build_debug=no
		export qbt_optimise_strip=no
		```
