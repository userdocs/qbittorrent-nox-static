...
set_default_values() {
	...
	#
	MODULES=("all" "install" "zlib" "icu" "openssl" "boost" "qtbase" "qttools" "libtorrent" "qbittorrent") # Define our list of available modules in an array.
	#
...
#####################################################################################################################################################
check_dependencies() {
	REQUIRED_PKGS=("bash" "bash-completion" "build-base" "curl" "pkgconf" "autoconf" "automake" "libtool" "git" "perl" "python${qb_python_version}" "python${qb_python_version}-dev" "py${qb_python_version}-numpy" "linux-headers") # Define our list of required core packages in an array.
	#
	...
	if [[ "${deps_installed}" = 'no' ]]; then
		if [[ "$(id -un)" = 'root' ]]; then
			...
			#
			apk add "${CHECKED_REQUIRED_PKGS[@]}" --repository="$CDN_URL"
			#
			...
			#
			echo -e "${tn}apk add ${CHECKED_REQUIRED_PKGS[*]}${tn}"
			#
			exit
		fi
	fi
