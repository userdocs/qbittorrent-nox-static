...
_set_module_urls() {
	...
	github_tag[icu]="$(_git_git ls-remote -q -t --refs "${github_url[icu]}" | awk '/\/release-/{sub("refs/tags/release-", "");sub("(.*)(-[^0-9].*)(.*)", ""); print $2 }' | awk '!/^$/ && !/rc/ && /^[0-9]+[-.]?[0-9]+[-.]?[0-9]*$/' | sort -rV | head -n 1)"
	...
	source_archive_url[icu]="https://github.com/unicode-org/icu/releases/download/release-${github_tag[icu]}/icu4c-${app_version[icu]/-/_}-src.tgz"
